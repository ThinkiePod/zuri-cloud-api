...
from models.v1 import Content, Device, DeviceCommand, UsageAnalytics
from schemas.v1 import ContentCreate, DeviceCommandRequest, DeviceHeartbeat, DeviceRegister, DeviceSettings, PlaybackCommand, UsageAnalyticsCreate, WiFiProvisionUpdate
...
@router.post("/devices/register", tags=["Device Management"], summary="Register a new device", deprecated=True)
async def register_device_v1(device_data: DeviceRegister, db: Session = Depends(get_db)):
...
@router.post("/devices/{device_id}/heartbeat", tags=["Device Management"], summary="Device heartbeat", deprecated=True)
async def device_heartbeat_v1(device_id: str, heartbeat: DeviceHeartbeat, db: Session = Depends(get_db)):
...
@router.get("/devices", tags=["Device Management"], summary="List all devices", deprecated=True)
async def get_devices_v1(user_id: Optional[str] = None, db: Session = Depends(get_db)):
...
@router.post("/devices/{device_id}/pair", tags=["Device Management"], summary="Pair device with user", deprecated=True)
async def pair_device_v1(device_id: str, user_data: dict, db: Session = Depends(get_db)):
...
@router.patch("/devices/{device_id}/wifi", tags=["Device Management"], summary="Update WiFi provisioning status", deprecated=True)
async def update_wifi_provisioning_v1(
    device_id: str,
...
# Content Management
@router.get("/content/library", tags=["Content Management"], summary="Get content library", deprecated=True)
async def get_content_library_v1(
...
@router.post("/content/library", tags=["Content Management"], summary="Add content to library", deprecated=True)
async def add_content_v1(content_data: ContentCreate, db: Session = Depends(get_db)):
...
@router.delete("/content/library/{content_id}", tags=["Content Management"], summary="Delete content", deprecated=True)
async def delete_content_v1(content_id: str, db: Session = Depends(get_db)):
...
# Device Control
@router.post("/devices/{device_id}/command", tags=["Device Control"], summary="Send command to device", deprecated=True)
async def send_device_command_v1(
    device_id: str, 
...
@router.post("/playback/play", tags=["Device Control"], summary="Play content on device", deprecated=True)
async def play_content_v1(playback: PlaybackCommand, db: Session = Depends(get_db)):
    """Play specific content on a device."""
    return await send_device_command_v1(
        playback.device_id,
...
@router.post("/playback/stop", tags=["Device Control"], summary="Stop playback", deprecated=True)
async def stop_playback_v1(
...
@router.post("/devices/{device_id}/settings", tags=["Device Control"], summary="Update device settings", deprecated=True)
async def update_device_settings_v1(
    device_id: str, 
...
# Analytics
@router.post("/analytics/usage", tags=["Analytics"], summary="Log usage analytics", deprecated=True)
async def log_usage_analytics_v1(
...
@router.get("/analytics/usage/{device_id}", tags=["Analytics"], summary="Get device usage analytics", deprecated=True)
...
async def device_websocket_v1(websocket: WebSocket, device_id: str):
...
async def mobile_websocket_v1(websocket: WebSocket):
...
# System endpoints
@router.get("/health", tags=["System"], summary="Health check", deprecated=True)
...
@router.get("/stats", tags=["System"], summary="System statistics", deprecated=True)
async def get_system_stats_v1(db: Session = Depends(get_db)):
...
@router.get("/", response_class=HTMLResponse, tags=["System"], summary="API Documentation Home", include_in_schema=False, deprecated=True)
async def home_page_v1(request: Request):
...
@router.get("/docs", response_class=HTMLResponse, include_in_schema=False, deprecated=True)
async def custom_swagger_ui_html_v1(request: Request):
    navbar_html, footer_html = load_navbar_and_footer_html()
...
@router.get("/redoc", response_class=HTMLResponse, include_in_schema=False, deprecated=True)
async def custom_redoc_html_v1(request: Request):
    navbar_html, footer_html = load_navbar_and_footer_html()
    custom_css = add_custom_color("#f4edf8")
